...
def permute_topK_test(
    ...
    permute_input = torch.rand((num_token, hidden_size), dtype=dtype).cuda()
    ...
    permute_output, sorted_indices = permute(permute_input, indices, num_topK)
    nvtx.range_pop()
    ...
    assert torch.allclose(permute_output, new_permute_output)
    ...
    if torch.allclose(permute_input.grad, new_permute_input.grad) == False:
    ...
    if torch.allclose(unpermute_output, new_unpermute_output) == False:
    ...
    if torch.allclose(unpermute_input.grad, new_unpermute_input.grad) == False:
    ...
    # old impl
    old_permute_input = permute_input.detach().repeat(num_topK, 1)
    ...
    if BENCHMARK:
        ...
        t = perf_test_cuda_kernel(lambda: permute_output.backward(permute_bwd_input, retain_graph=True))
        print(f"pytorch bwd: {t:.3f} ms")
        t = perf_test_cuda_kernel(
            lambda: new_permute_output.backward(permute_bwd_input, retain_graph=True))
        print(f"new     bwd: {t:.3f} ms")
        t = perf_test_cuda_kernel(
            lambda: old_permute_output.backward(permute_bwd_input, retain_graph=True))
        ...
        t = perf_test_cuda_kernel(
            lambda: unpermute_output.backward(unpermute_bwd_input, retain_graph=True))
        print(f"pytorch bwd: {t:.3f} ms")
        t = perf_test_cuda_kernel(
            lambda: new_unpermute_output.backward(unpermute_bwd_input, retain_graph=True))
        print(f"new     bwd: {t:.3f} ms")
        t = perf_test_cuda_kernel(
            lambda: old_unpermute_outputs.backward(old_unpermute_bwd_input, retain_graph=True))
        print(f"old     bwd: {t:.3f} ms")
