...
moe_group_gemm_backward_op = torch.ops.moe_unit_ops.moe_group_gemm_backward_op
...
if __name__ == "__main__":
  ...
  # (hs, tokens) x (tokens, inter) → (hs, inter)
  # (inter, tokens) x (tokens, hs) → (inter, hs)

  #                       group, tokens, inter,  hs
  ...
  for gemm_case in gemm_cases:
    ...
    for config_id in range(1, 6):
      ...
      for stage_id in range(2, stages[config_id - 1] + 1):
        ...
        mat_x = torch.randn([num_tokens, hidden], dtype=dtype, device="cuda")
        may_y_grad = torch.randn([num_tokens, inter], dtype=dtype, device="cuda")
        
        nvtx.range_push("warmups")
        for _ in range(10):
          gemm1_output = moe_group_gemm_backward_op(mat_x, expert_for_rows, may_y_grad, num_experts)
        nvtx.range_pop()
  
        t = triton.testing.do_bench(lambda: moe_group_gemm_backward_op(mat_x, expert_for_rows, may_y_grad, num_experts))
